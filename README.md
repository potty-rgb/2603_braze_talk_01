...
| 단계 | 이름 | 설명 |
|------|------|------|
| **Step 1** | 타입 선택 | 버튼 O / 버튼 X 카드를 클릭하여 시작 |
| **Step 2** | 정보 입력 | 템플릿 원문, 발송 원문, 템플릿 코드 등 입력 |
| **Step 3** | 결과 확인 | 변수 매칭 결과 및 Liquid 코드 확인 · 복사 |
| **Step 4** | 오류 진단 | 발송 오류 진단 및 자동 수정 (3열 레이아웃으로 동시 표시) |

> Step 2 이후에는 **정보 입력 | 결과 확인 | 오류 진단** 3열 레이아웃으로 한눈에 확인할 수 있습니다.
...
| 모드 | 접근 | 특징 |
|------|------|------|
| **Standalone 모드** | 홈화면 "오류 해결하기" 카드 클릭 | Liquid 코드 + 오류 메시지를 순서대로 입력 |
| **Connected 모드** | Step 2 이후 3열 레이아웃 우측 패널 | 생성된 Liquid 코드가 자동 연결, 오류 메시지만 입력 |
...
- Standalone 모드에서는 오류 메시지와 Liquid 코드 모두 입력했는지 검증 후 AI 안내
...
```
src/
├── App.tsx                      # 메인 레이아웃 (4단계 + 3열 구조)
├── components/
│   ├── TemplateTypeSelector.tsx  # 홈화면 카드 + 응원 메시지 롤링
│   ├── StepIndicator.tsx         # 4단계 진행 표시기
│   ├── InputForm.tsx             # Step 2 정보 입력 폼
│   ├── ResultDisplay.tsx         # Step 3 결과 표시 + 복사
│   └── ErrorDiagnoser.tsx        # 오류 진단 (standalone/embedded 모드)
├── utils/
